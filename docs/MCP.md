...
| **Workers & Queue** | Offline processing of large files + AI generation using the Groq + SERPER research pipeline. | RQ worker bootstrap in `worker.py`; queue producer in `producer.py`; job executor/streaming helpers coordinating Supabase storage chunks and Groq completions in `jobs.py` & `file_streaming.py`; Groq research orchestration in `groq_pipeline.py`.【F:backend/app/worker.py†L1-L8】【F:backend/app/producer.py†L1-L36】【F:backend/app/jobs.py†L1-L200】【F:backend/app/file_streaming.py†L1-L127】【F:backend/app/groq_pipeline.py†L1-L178】 |
...
2. **Background processing:** Worker pulls job, streams raw chunks to Supabase `inputs` bucket, iterates rows, runs SERPER search + Groq models to generate openers, and uploads outputs back to Supabase while logging progress + credit usage in SQLite ledger. Failures surface via job logs and storage helpers.【F:backend/app/jobs.py†L39-L200】【F:backend/app/file_streaming.py†L1-L127】【F:backend/app/groq_pipeline.py†L1-L178】【F:backend/app/db.py†L113-L162】
...
- **Core secrets:** `.env` must provide Supabase service key/URL, Groq & SERPER API keys, Stripe API keys, and allowed frontend origins consumed by FastAPI during startup.【F:backend/app/main.py†L33-L149】【F:backend/app/supabase_client.py†L13-L21】【F:backend/app/groq_pipeline.py†L1-L74】
...
- **Investigating stuck jobs:**
  ...
  4. If Groq calls fail or SERPER returns empty payloads, check worker logs for fallback reasons recorded per row and retry after adjusting service context or email quality.【F:backend/app/jobs.py†L600-L724】【F:backend/app/groq_pipeline.py†L120-L178】
...
- **Operational telemetry:**
  ...
  - Add health endpoints or Prometheus exporters to monitor Redis depth, worker throughput, and Groq/SERPER latency; foundation exists in queue helpers for instrumentation.【F:backend/app/jobs.py†L22-L200】
...
2. **Improve worker resilience:** Add retry policies around SERPER + Groq calls and Supabase uploads to handle transient failures observed in `_upload_to_storage`.【F:backend/app/jobs.py†L66-L142】【F:backend/app/groq_pipeline.py†L94-L178】
