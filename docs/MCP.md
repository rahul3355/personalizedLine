...
| **Workers & Queue** | Offline processing of large files, Serper enrichment, and Groq-based personalization. | RQ worker bootstrap in `worker.py`; queue producer in `producer.py`; job executor/streaming helpers coordinating Supabase storage chunks and Groq completions in `jobs.py` & `file_streaming.py`; Groq pipeline + compatibility helpers in `personalization_pipeline.py` and `gpt_helpers.py`.【F:backend/app/worker.py†L1-L8】【F:backend/app/producer.py†L1-L36】【F:backend/app/jobs.py†L1-L200】【F:backend/app/file_streaming.py†L1-L127】【F:backend/app/personalization_pipeline.py†L1-L210】【F:backend/app/gpt_helpers.py†L1-L65】 |
...
2. **Background processing:** Worker pulls job, streams raw chunks to Supabase `inputs` bucket, iterates rows, enriches each email via Serper + Groq to capture `user_info` and personalized openers, then uploads outputs back to Supabase while logging progress + credit usage in SQLite ledger. Failures surface via job logs and storage helpers.【F:backend/app/jobs.py†L39-L200】【F:backend/app/file_streaming.py†L1-L127】【F:backend/app/personalization_pipeline.py†L1-L210】【F:backend/app/db.py†L113-L162】
...
- **Core secrets:** `.env` must provide Supabase service key/URL, Groq API key, Serper API key, Stripe API keys, and allowed frontend origins consumed by FastAPI during startup.【F:backend/app/main.py†L33-L149】【F:backend/app/supabase_client.py†L13-L21】【F:backend/app/personalization_pipeline.py†L70-L125】
