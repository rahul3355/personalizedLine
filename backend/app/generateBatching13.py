...
def ai_estimate_news(company_name, industry, description, headline=""):
    ...
    try:
        ...
        resp = openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Generate a short 2-3 sentence news-style update about the company. Stay general and plausible, avoid numbers or fake data."},
...
def regenerate_guaranteed_trend(row):
    ...
    try:
        ...
        resp = openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Write one sentence about this company’s trend/news using the provided data. Must include company name. No filler or disclaimers."},
...
if __name__ == "__main__":
    process_excel_in_batches("p3.xlsx", batch_size=10, output_file="p3_with_openers12.xlsx")
