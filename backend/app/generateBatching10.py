...
def truncate(value, limit=1000):
    """Ensure value is a string and truncate safely."""
...
def build_guaranteed_trend(row):
    """Ensure guaranteed trend using all available fields if AI + scraping fail."""
    summ = row.get("ai_summarized_news")
    est = row.get("ai_estimated_news")
    clean = row.get("clean_news_data")
    raw = row.get("raw_news_data")
    if summ: return summ
    if est: return est
    if clean: return clean
    if raw: return raw

    # fallback: use all non-empty row fields
    fields = []
    for col, val in row.items():
        val_str = truncate(val, 200)
        if val_str and val_str.lower() not in ["0", "nan", "none"]:
            fields.append(f"{col}: {val_str}")
    if fields:
        return f"Based on company data ({' | '.join(fields[:10])}), the company is adapting to industry changes and pursuing growth."
    return "This company is navigating industry shifts while focusing on growth."
...
if __name__ == "__main__":
    process_excel_in_batches("p3.xlsx", batch_size=4, output_file="p3_with_openers9.xlsx")
