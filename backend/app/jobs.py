...
def _request_groq_completion(prompt: str, email_value: str) -> str:
    if not GROQ_API_KEY:
        print(
            f"[Worker] SIF research skipped for {email_value}: GROQ_API_KEY missing"
        )
        return ""
    ...
    try:
        ...
        print(
            f"[Worker] SIF research Groq call failed for {email_value}: {exc}"
        )
        return ""
    ...
    try:
        ...
        print(
            f"[Worker] SIF research Groq response parse error for {email_value}: {exc}"
        )
        return ""

    if not output:
        print(f"[Worker] SIF research Groq response empty for {email_value}")
        return ""
