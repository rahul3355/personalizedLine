...
def summarize_article(text: str) -> str:
    global OPENAI_TOKENS, OPENAI_TIME
    if not text.strip():
        return ""
    try:
        start = time.time()
...
def ai_estimate_news(company_name, industry, description, headline=""):
    # OpenAI for estimation too (safer than raw HTML fallback)
    global OPENAI_TOKENS, OPENAI_TIME
...
def generate_openers_batch(batch_rows):
    global DEEPSEEK_TOKENS, DEEPSEEK_TIME
    inputs_text = []
    ...
    DEEPSEEK_TOKENS += usage.total_tokens
...
def process_excel_in_batches(file_path, batch_size=4, output_file="output.xlsx"):
    ...
    # Final summary
    total_tokens = OPENAI_TOKENS + DEEPSEEK_TOKENS
    print("\n=== Job Summary ===")
    print(f"OpenAI tokens: {OPENAI_TOKENS}")
    print(f"DeepSeek tokens: {DEEPSEEK_TOKENS}")
    ...
    print(f"Average OpenAI tokens per row: {OPENAI_TOKENS/total_rows:.2f}")
    print(f"Average DeepSeek tokens per row: {DEEPSEEK_TOKENS/total_rows:.2f}")
    print(f"Average OpenAI tokens per batch: {OPENAI_TOKENS/max(batch_count,1):.2f}")
    print(f"Average DeepSeek tokens per batch: {DEEPSEEK_TOKENS/max(batch_count,1):.2f}")
    print(f"Results saved to: {output_file}")


if __name__ == "__main__":
    process_excel_in_batches("p3.xlsx", batch_size=4, output_file="p3_with_openers11.xlsx")
