...
# === Default service context (kept for consistency in USER_TEMPLATE) ===
...
# === Opener Generation Rules (DeepSeek, pain-point style refined) ===
BATCH_PROMPT_RULES = (
...
# === GPT Enrichment for News/Funding ===
def enrich_news_statement(company_name, headline, snippet=""):
    ...
    try:
        ...
        resp = openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Rewrite into one clear, company-specific sentence. Mention the company explicitly. No filler, no generic industry commentary."},
...
def lookup_recent_news(company_name: str, company_domain: str = None):
    sources = []
    # Try company feeds
    if company_domain:
        candidates = [
        ...
        for rss_url in candidates:
            ...
            if entries:
                e = entries[0]
                enriched = enrich_news_statement(company_name, e.title, e.get("summary",""))
                return e.title, e.link, enriched
    # Try Google News domain
    if company_domain:
...
def lookup_funding_news(company_name: str):
...
def ai_estimate_news(company_name, industry, description, headline=""):
    ...
    try:
        ...
        resp = openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Write a short 2-3 sentence news-style update about the company. Avoid numbers or fake data. Must include company name."},
                {"role": "user", "content": context}
            ],
            max_tokens=800
        )
...
def regenerate_guaranteed_trend(row):
    ...
    try:
        ...
        resp = openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Write exactly two sentences of company-specific news or trend. Must include company name. No filler, no disclaimers, no mentions of missing context."},
    ...
    for idx, row in enumerate(batch_rows, start=1):
        inputs_text.append(
            f"Input {idx}:\n" + USER_TEMPLATE.format(
                ...
                recent_news=truncate(row.get("Recent News Enriched"), 800),
                funding_news=truncate(row.get("Funding News Enriched"), 800),
...
def process_excel_in_batches(file_path, batch_size=8, output_file="output.xlsx"):
    ...
    for start in range(0, total_rows, batch_size):
        ...
        for i, row in batch.iterrows():
            ...
            recent_head, recent_url, recent_enriched = lookup_recent_news(company_name, company_domain)
            funding_head, funding_url, funding_enriched = lookup_funding_news(company_name)

            df.at[i, "Recent News"] = recent_head
            df.at[i, "Recent News URL"] = recent_url
            df.at[i, "Recent News Enriched"] = recent_enriched
            df.at[i, "Funding News"] = funding_head
            df.at[i, "Funding News URL"] = funding_url
            df.at[i, "Funding News Enriched"] = funding_enriched

            raw, clean, summ, est = "", "", "", ""
            if recent_url:
                raw, clean = fetch_article_content(recent_url)
                summ = summarize_article(clean or raw)
            if not summ:
                est = ai_estimate_news(company_name, row.get("Industry",""), row.get("Company Short Description",""), recent_head)
...
if __name__ == "__main__":
    process_excel_in_batches("p3.xlsx", batch_size=8, output_file="p3_with_openers17.xlsx")
