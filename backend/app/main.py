...
@app.post("/debug/check-reward")
def debug_check_reward(current_user: AuthenticatedUser = Depends(get_current_user)):
    """
    Manually trigger the reward check logic for the current user.
    This helps verify if the logic works when run in the API process.
    """
    try:
        user_id = current_user.user_id
        print(f"[Debug] Checking reward for user {user_id}")
        
        # 1. Check current status
        p_res = supabase.table("profiles").select("created_at, welcome_reward_status").eq("id", user_id).single().execute()
        ...
        if status == "locked" and created_at_str:
            ...
            is_within_time = now < created_at + timedelta(days=7)
            
            results["checks"].append(f"Time check: {now} < {created_at + timedelta(days=7)} = {is_within_time}")
            ...
            if is_within_time:
                ...
                results["checks"].append(f"Total used: {total_used}")
                results["total_used"] = total_used

                if total_used >= 500:
                    supabase.table("profiles").update({
                        "welcome_reward_status": "unlocked"
                    }).eq("id", user_id).execute()
                    results["outcome"] = "UNLOCKED"
                else:
                    results["outcome"] = f"Not enough credits ({total_used}/500)"
            else:
                results["outcome"] = "Time expired"
        else:
            results["outcome"] = "Not locked or missing created_at"
