...
# === Default service context (used if row has none) ===
DEFAULT_SERVICE_CONTEXT = "I help businesses grow with modern solutions."

# === Prompt rules for DeepSeek ===
BATCH_PROMPT_RULES = (
    "You are writing cold email openers that feel human and conversational.\n\n"
    "Task:\n"
    "For each input company, write a personalized two-sentence cold email opener (18–25 words total).\n\n"
    "Rules:\n"
    "- If `Kimi News` is provided, base the first sentence on that news.\n"
    "- If no news is available, base the first sentence on other details (industry, description, technologies, keywords, or role).\n"
    "- The second sentence must surface a subtle pain or friction implied by that detail, framed in a way that connects to the provided service context, without describing the service itself.\n"
    "- Tone: plain, human, like writing to a colleague. Not salesy, not flattering, not formal.\n"
    "- Do not invent numbers or fake data.\n"
    "- Do not ask questions. Do not explain your reasoning.\n\n"
    "Output format:\n"
    "Numbered list (1., 2., 3., …), with exactly one opener per company."
)
...
if __name__ == "__main__":
    process_excel_in_batches("p3.xlsx", batch_size=6, output_file="p3_with_kimi_batched5.xlsx")
